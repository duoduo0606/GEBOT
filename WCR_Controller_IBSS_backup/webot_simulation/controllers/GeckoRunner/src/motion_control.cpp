    ...
    void Motion_control::fly_torque(int fly_leg1, int fly_leg2, int sup_leg1, int sup_leg2)
    {
      ...
      if (sinario < 3)
      {
        ...
        foot_F_f_and_h_fly(0) = 0;
        foot_F_f_and_h_fly(1) = 0;
        foot_F_f_and_h_fly(2) = kzf * (lf_pos_z - zcf) + dzf * (lf_lnr_vel(2) - zcfdot);
        foot_F_f_and_h_fly(3) = 0;
        foot_F_f_and_h_fly(4) = 0;
        foot_F_f_and_h_fly(5) = kzh * (rh_pos_z - zch) + dzh * (rh_lnr_vel(2) - zchdot);
      ...
      if (sinario > 1)
      {
        ...
        cout << "左后脚期望的X位置 xch" << xch << endl;
        cout << "左后脚期望的Y位置 ych" << ych << endl;
        cout << "左后脚期望的Z位置 zch" << zch << endl;
        
        // cout << "右前脚的实际Z位置" << rf_pos_z << endl;
        cout << "左后脚的实际X位置" << lh_pos_x << endl;
        cout << "左后脚的实际Y位置" << lh_pos_y << endl;
      ...
      for (int i = 0; i < 3; i++)
        { 
          ...
          //limb_motors[fly_leg2][i]->setPosition(0); 
